...
std::vector<std::vector<double>> fftCalculation(std::vector<double> data){
    ...
    for(auto i = 0; i < N_size/2; i++){
        ...
        out[1][i] = 10.0*log10(1.0/N_size*(std::abs(pow(fft_out[i][0], 2.0) + pow(fft_out[i][1], 2.0))));
    }
    fftw_destroy_plan(p);
    fftw_cleanup();
    return out;
...
void MyObject::doData(){
    std::vector<std::vector<double>> out = {std::vector<double>(N_size), std::vector<double>(N_size)};
    std::normal_distribution<double> dist_amp(0.0, 0.1);
    std::normal_distribution<double> dist_freq(100.0, 0.01);
...
void MyObject::doFFT(std::vector<std::vector<double>> *in){
    std::vector<std::vector<double>> out = {std::vector<double>(N_size), std::vector<double>(N_size)};
    std::vector<std::vector<double>> dataFFT = fftCalculation((*in)[1]);
    for (int i=0; i< N_size; ++i) {
        out[0][i] = dataFFT[0][i];
        out[1][i] = dataFFT[1][i];
    }
